...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    A = self.num_anchors
    layer_out = self.pred_layer(features) #output of region proposal layer
    out = layer_out.reshape(B,A,6,H,W)

    if mode == 'train':
      M = pos_anchor_coord.shape[0]
      pos_ancs = self._extract_anchor_data(out,pos_anchor_idx)
      neg_ancs = self._extract_anchor_data(out,neg_anchor_idx)
      #print(pos_ancs.shape)
     # print(pos_ancs[1])
      conf_scores = torch.cat((pos_ancs[:,:2],neg_ancs[:,:2]))
      offsets = pos_ancs[:,2:]
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
   ...
   # IoU_mat = IoU(proposals, bboxes)
    ...
    conf_scores, offsets, proposals = self.prop_module(features, activated_anc_coord, activated_anch_idx, negative_anch_idx)
    pos_anchor_idx = activated_anch_idx
    neg_anchor_idx = negative_anch_idx
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
      ...
      A = self.anchor_list
      grid = GenerateGrid(B)
      self.anchor_list = self.anchor_list.to(images.dtype).to(images.device) ###change type??
      anchors = GenerateAnchor(self.anchor_list,grid)

      conf_scores, offsets = self.prop_module(features)
     # print(conf_scores.shape) #conf_scores - [B,_, 2, H, W]
      conf_probs = torch.sigmoid(conf_scores[:,:,0,:,:])
      offsets = offsets.permute(0,1,3,4,2)
      proposals = GenerateProposal(anchors, offsets)
      conf_probs = conf_probs.permute((0,2,3,1)).reshape(B,-1)
      proposals = proposals.permute((0,2,3,1,4)).reshape(B,-1,4)
      for i in range(B):

        #only keep proposals when above the threshold
        mask = conf_probs[i] > thresh
        curr_conf_probs = conf_probs[i][mask] ##only keep certain conf_scores of i-th image
        curr_proposals = proposals[i][mask,:]

        #nms
        mask_nms = torchvision.ops.nms(curr_proposals, curr_conf_probs, iou_threshold=nms_thresh)
        final_proposals.append(curr_proposals[mask_nms,:])
        final_conf_probs.append(curr_conf_probs[mask_nms].unsqueeze(1))
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    batch_idx = batch_idx.to(proposals.device).to(proposals.dtype)
    boxes = torch.zeros((proposals.shape[0],5), device=proposals.device, dtype=proposals.dtype)
    # print(proposals.squeeze().shape)
    #print(boxes.shape)
    boxes[:,0] = batch_idx
    boxes[:,1:] = proposals.squeeze()

    roi_aligned = torchvision.ops.roi_align(features, boxes, (self.roi_output_w, self.roi_output_h))
    roi_meanpooled = torch.mean(roi_aligned,dim=(2,3)) ##meanpooled features

    ###send roi features through classification 
    cls_scores = self.cls_layer(roi_meanpooled)
    cls_loss = nn.functional.cross_entropy(cls_scores, GT_class, reduction='sum') * 1./B
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    aligned_feats = torchvision.ops.roi_align(features, final_proposals,(self.roi_output_w, self.roi_output_h))
    #print(len(final_proposals), len(final_conf_probs), B, len(features), images.shape)
    #print(final_proposals[0]). ##size = (4,1)
    # print((aligned_feats.shape)). ###Shape [51,1280,2,2]
    #print(features[1].shape) ### [1280,7,7]
    pooled_feats = torch.mean(aligned_feats,dim=(2,3)) 
    #print(pooled_feats.shape)  #### [51,1280]    
    classification_scores = self.cls_layer(pooled_feats)
   # print(classification_scores[1])
    class_pred = torch.max(classification_scores,dim=1)[1].unsqueeze(1) #
    
    final_class = []
    idx = 0
    for p in range(B):
      props_curr_im = len(final_proposals[p])
      final_class.append(class_pred[idx:idx+props_curr_im])
      idx += props_curr_im
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
