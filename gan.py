...
def sample_noise(batch_size, noise_dim, dtype=torch.float, device='cpu'):
  ...
  noise = 2*(noise - 0.5) #shift and rescale
  ##############################################################################
  #                              END OF YOUR CODE                              #
  ##############################################################################
...
def discriminator_loss(logits_real, logits_fake):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  device_ = logits_real.device
  loss_fake = BCE_loss(logits_fake, torch.zeros_like(logits_fake, device=device_))
  loss_real = BCE_loss(logits_real, torch.ones_like(logits_real, device=device_))
...
def generator_loss(logits_fake):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  device_ = logits_fake.device
  loss = BCE_loss(logits_fake, torch.ones_like(logits_fake, device=device_))
  ##############################################################################
  #                              END OF YOUR CODE                              #
  ##############################################################################
  return loss
...
def ls_discriminator_loss(scores_real, scores_fake):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  loss = 0.5*(scores_real - torch.ones_like(scores_real))**2
  loss += 0.5*(scores_fake**2)
