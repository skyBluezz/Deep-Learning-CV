...
def three_layer_convnet(x, params):
  ...
  #x = flatten(x)
  x = F.conv2d(x,weight=conv_w1,bias=conv_b1,padding=2)#conv_b2.shape[0], conv_w1.shape[2], conv_w1.shape[3])
  x = F.relu(x)
  x = F.conv2d(x,weight=conv_w2,bias=conv_b2,padding=1)#fc_b.shape[0], conv_w2.shape[2], conv_w2.shape[3])
  x = F.relu(x)
  x = flatten(x)
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  conv_w1 = nn.init.kaiming_normal_(torch.zeros((channel_1, C, kernel_size_1, kernel_size_1), dtype=dtype, device=device))
  conv_w1.requires_grad = True
  conv_b1 = torch.zeros(channel_1).to(dtype).to(device=device)
  #print(conv_w1.shape)
  conv_b1.requires_grad = True

  conv_w2 = nn.init.kaiming_normal_(torch.zeros((channel_2, channel_1, kernel_size_2, kernel_size_2), dtype=dtype, device=device))
  conv_w2.requires_grad = True
  conv_b2 = torch.zeros(channel_2).to(dtype).to(device=device)
  conv_b2.requires_grad = True

  fc_w = nn.init.kaiming_normal_(torch.zeros((num_classes, channel_2*H*W), dtype=dtype, device=device))
  fc_w.requires_grad = True
  fc_b = torch.zeros(num_classes).to(dtype).to(device=device)
...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    self.conv1 = nn.Conv2d(in_channel, channel_1,K1,padding=2)
    self.conv2 = nn.Conv2d(channel_1, channel_2,K2,padding=1)
...
def initialize_three_layer_conv_part4():
  ...
  ####################################################################################
  # Replace "pass" statement with your code
  model = nn.Sequential( nn.Conv2d(C, channel_1, kernel_size_1, padding = 2),
                        nn.ReLU(),
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential( nn.BatchNorm2d(Cin),
                              nn.ReLU(),
                              nn.Conv2d(Cin, Cout, K1, padding=1,stride=stride),
                              nn.BatchNorm2d(Cout),
                              nn.ReLU(),
                              nn.Conv2d(Cout,Cout,K1,padding=1)
                              )
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
  ...
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if(downsample):
      ...
      self.shortcut = nn.Sequential(
          nn.Conv2d(Cin, Cout, 1, stride=stride))
    else:
      if Cin == Cout:
        self.shortcut = nn.Sequential(
            nn.Identity())
    ...
    #   self.shortcut = nn.Conv2d(Cin, Cout, 1, padding=0, stride=stride)
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    network = [ResNetStem(Cin=Cin,Cout=stage_args[0][0])]
    len_ = len(stage_args)
    for stage_arg in stage_args:
      network.append(ResNetStage(*stage_arg,block=block))
    #network.append(torch.nn.AdaptiveAvgPooling2d(1,1))
    self.cnn=nn.Sequential(*network)
  ...
  def forward(self, x):
    ...
    # Replace "pass" statement with your code
    x = self.cnn(x)
    scores = F.avg_pool2d(x, x.shape[2])
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.shortcut = nn.Sequential()
    if (Cin != Cout) or downsample:
      self.shortcut = nn.Sequential(nn.Conv2d(Cin, Cout, 1, padding=0, stride=stride))
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
