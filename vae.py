...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # Replace "pass" statement with your code
        N, _, H, W = x.shape
        #print(x.reshape(N,-1).shape,c.shape)
        _input_ = torch.cat((x.reshape(N,-1),c),dim=1)
        encoded = self.encoder(_input_)
        mu = self.mu_layer(encoded)
        logvar = self.logvar_layer(encoded)
        z = reparametrize(mu,logvar)
...
def reparametrize(mu, logvar):
    ...
    ################################################################################################
    # Replace "pass" statement with your code
    z = mu + torch.exp(0.5*logvar)*torch.normal(0, 1,size=mu.shape).to(mu.device)
    #z = mu + torch.sqrt(torch.exp(logvar))*torch.normal(mu, torch.exp(logvar))
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    term = 1 + logvar - torch.mul(mu,mu) - torch.exp(logvar)
    Dkl_loss = -0.5*torch.sum(term,dim=1)
    recon_loss = nn.functional.binary_cross_entropy(x_hat,x)
    recon_loss = torch.mean(recon_loss,dim=0)*H*W

    loss = torch.sum(recon_loss) + torch.sum(Dkl_loss)/N
    ################################################################################################
    #                            END OF YOUR CODE                                                  #
    ################################################################################################
    return loss
