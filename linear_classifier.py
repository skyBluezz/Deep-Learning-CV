...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #Below we collect the scores of corr labels
  correct_scores = scores[torch.arange(num_train),y]

  #f(x) at each, or the 'margin' matrix
  fx_matrix = scores - correct_scores.view(-1,1) + 1
  fx_matrix[fx_matrix < 0] = 0
  
  #Set the yi=j entries to zero
  fx_matrix[torch.arange(num_train),y] = 0

  #Sum over rows first??
  loss = torch.sum(fx_matrix)/num_train
  loss += reg * torch.sum(W * W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  ...
  #############################################################################
  # Replace "pass" statement with your code
  grad_margins = fx_matrix
  grad_margins[grad_margins > 0] = 1
  class_sums = torch.sum(fx_matrix,dim=1)
  grad_margins[torch.arange(num_train), y] = -class_sums.view(1,-1)
  dW = torch.mm(torch.transpose(X,0,1),grad_margins) / num_train
  dW += 2*reg*W
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [8e-3, 9e-3, 1e-2, 1.8e-2]
  regularization_strengths = [8e-3, 1e-2, 5e-2,7e-2]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  loss_history = cls.train(X_train, y_train, lr, reg, num_iters=200, batch_size=200,verbose=True)
...
def softmax_loss_naive(W, X, y, reg):
  ...
  stability_term = torch.max(scores,dim=1)[0] 
  scores -= stability_term.view(-1,1)
  
  for i in range(num_train):
      score = scores[i]
      sum_denom = torch.sum(torch.exp(score))
      func = lambda t: torch.exp(score[t]) / sum_denom
      loss -= torch.log(func(y[i]))
  
      for c in range(num_classes):
          dW[:,c] += X[i]*(func(c) - 1*(c == y[i]))
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores -= stability_term.view(-1,1)
  sum_denom = torch.sum(torch.exp(scores),dim=1) 
  logsum = torch.log(sum_denom)
  loss = (logsum - scores[torch.arange(num_train), y])/num_train
  loss = torch.sum(loss)
  loss += reg*torch.sum(W*W)
  
  alpha = torch.transpose(torch.exp(scores),0,1) / sum_denom
  alpha = torch.transpose(alpha,0,1)
  alpha[torch.arange(num_train),y] -= 1
  dW = torch.mm(torch.transpose(X,0,1),alpha)
  dW /= num_train
  dW += 2*reg*W
  #############################################################################
  #                          END OF YOUR CODE                                 #
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-2, 1.8e-2, 4e-2, 7e-2]
  regularization_strengths = [2e-3, 5e-3, 8e-3, 1e-2]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
