...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    # Pad x_pad
    ####should only pad last two...
    x_pad = torch.nn.functional.pad(x, (pad,pad, pad, pad, 0, 0), 'constant', value=0).to()
        ...
        for f in range(F):
            for h in range(0, H_prime):
                for i in range(0, W_prime):
                    out[n, f, h, i] = torch.sum((x_pad[n, :, h*stride:h*stride+HH, i*stride:i*stride+WW] * w[f,...])) + b[f]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b, conv_param)
  ...
  #CHECK IF PADDING SHOULD BE (0,0,PAD,PAD)
    x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad, 0, 0), 'constant', value=0)
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    out = torch.zeros(N, C, Hpool, Wpool, dtype=x.dtype, device=x.device)
    for n in range(N):
      for c in range(C):
      ...
      for c in range(C):
        for i in range(H1):
          for j in range(W1):
            ...
            dpool_mask = torch.zeros(pool_height*pool_width, dtype=x.dtype, device=x.device)
            dpool_mask[torch.argmax(pool_region)] = 1
            yeah = torch.mul(dpool_mask.view(pool_height,pool_width), dout[n, c, i, j])
            dx[n,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width] +=  torch.mul(dpool_mask.view(pool_height,pool_width), dout[n, c, i, j])
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W1'] = torch.normal(mean=0, std=weight_scale, size=(num_filters, C, filter_size, filter_size)).to(dtype).to(device)
    self.params['b1'] = torch.zeros(num_filters).to(dtype).to(device)

    #conv out size 
    pad = (filter_size - 1) // 2
    HH = (H + 2 * pad - filter_size) + 1 #assume stride=1
    WW = (W + 2 * pad - filter_size) + 1

    #after pooling
    H_ = (HH - 2) // 2 + 1 #since 2x2 max pooling
    W_ = (WW - 2) // 2 + 1

    linear_dim = num_filters * H_ * W_

    self.params['W2'] = torch.normal(mean=0, std=weight_scale, size=(linear_dim, hidden_dim)).to(dtype).to(device)
    self.params['b2'] = torch.zeros(hidden_dim).to(dtype).to(device)
    self.params['W3'] = torch.normal(mean=0, std=weight_scale, size=(hidden_dim, num_classes)).to(dtype).to(device)
  ...
  def loss(self, X, y=None):
    ...
    dx3, dw3, db3 = Linear.backward(dloss, cache3)
    dx2, dw2, db2 = Linear_ReLU.backward(dx3, cache2)
    dx1, dw1, db1 = Conv_ReLU_Pool.backward(dx2, cache1)

    num_layers = len(self.params.keys()) // 2
    dw_dict = {'dw3':dw3,'dw2':dw2,'dw1':dw1}
    db_dict = {'db3':db3,'db2':db2,'db1':db1}
    
    for i in range(1,num_layers+1):
      grads['W' + str(i)] = dw_dict['dw'+str(i)] + 2*self.reg*self.params['W'+str(i)]
      grads['b' + str(i)] = db_dict['db'+str(i)]
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    # Replace "pass" statement with your code
    C,H,W = input_dims
    kernel_size = 3 #Kernel or filter size?
    pad = 1
    ...
    for i in range(1,self.num_layers+1):
      ...
      if(weight_scale == 'kaiming'):
        if(i==self.num_layers):
          ...
          self.params['W' + str(i)] = kaiming_initializer(linear_dim, num_classes, relu=False, device=device, dtype=dtype)
          self.params['b'+str(i)] = torch.zeros(num_classes).to(dtype).to(device)
        else:
        ...
        if(i==self.num_layers):
          num_pool = len(max_pools)
          linear_dim = num_filters[-1] * (H // (2 ** num_pool))* (W // (2 ** num_pool))
          self.params['W'+str(i)] = torch.normal(mean=0, std=weight_scale, size=(linear_dim, num_classes)).to(dtype).to(device)
          self.params['b'+str(i)] = torch.zeros(num_classes).to(dtype).to(device)
        else:
          self.params['W' + str(i)] =  weight_scale * torch.randn(num_filters[i-1], C, 3, 3, dtype=dtype, device=device)
          self.params['b'+ str(i)] = torch.zeros(num_filters[i-1], dtype=dtype, device=device)
          C = num_filters[i-1]
    # print('weight1 shape', self.params['W1'].shape)
    # print('weight2 shape', self.params['W2'].shape)
    # print('weight3 shape', self.params['W3'].shape)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    # last layer
    dloss, dw, db = Linear.backward(dloss, caches.pop())
    ...
    for i in range(0, self.num_layers-1):
      ...
      if self.num_layers- 2 - i in self.max_pools:
        # if maxpooling , conv-relu-pool    
        if self.batchnorm:
          dloss, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dloss, caches.pop())
          ...
          dloss, dw, db = Conv_ReLU_Pool.backward(dloss, caches.pop())
          grads['W' + str(self.num_layers -1 - i)] = dw + 2 * self.reg * self.params['W' + str(self.num_layers -1 - i)]
          grads['b' + str(self.num_layers -1 - i)] = db
      else:
        if self.batchnorm:
          dloss, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dloss, caches.pop())
          grads['W' + str(self.num_layers -1 - i)] = dw + 2 * self.reg * self.params['W' + str(self.num_layers -1 - i)]
          grads['b' + str(self.num_layers -1 - i)] = db
          grads['gamma' + str(self.num_layers -1 - i)] = dgamma
          grads['beta' + str(self.num_layers -1 - i)] = dbeta
        else:
          dloss, dw, db = Conv_ReLU.backward(dloss, caches.pop())
          grads['W' + str(self.num_layers -1 - i)] = dw + 2 * self.reg * self.params['W' + str(self.num_layers -1 - i)]
          grads['b' + str(self.num_layers -1 - i)] = db
...
def find_overfit_parameters():
  ...
  learning_rate = 8e-3
#best, 0.96
  #   weight_scale = 0.875e-3
  # learning_rate = 2.0905e-2
  ############################################################################
  #                             END OF YOUR CODE                             #
  ############################################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  solver = Solver(model, data_dict,
                  print_every=50, num_epochs=8, batch_size=128,
                  update_rule=adam,
                  lr_decay = 0.92,
                  optim_config={
                    'learning_rate': lr,
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
  ...
  if K is None:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    weight = math.sqrt(gain / Din) * torch.randn(Din, Dout, dtype=dtype, device=device) 
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  else:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    weight = math.sqrt(gain / (Din * K * K)) * torch.randn(Dout, Din, K, K, dtype=dtype, device=device) 
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  return weight
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      #######################################################################
      # Replace "pass" statement with your code
      mu = torch.mean(x,dim=0) #average each sample
      var = 1. * torch.sum((x-mu)**2,0) / N

      x_norm = (x - mu) / ((var+eps)**(1/2))
      y = gamma * x_norm + beta   #BN{y}

      running_mean = momentum * running_mean + (1 - momentum) * mu
      running_var = momentum * running_var + (1 - momentum) * var
      cache = {"running_mean":running_mean, "running_var":running_var, 'x_norm': x_norm, 'mu':  mu, 'var':var, "gamma": gamma, "eps": eps, "beta": beta, "x":x, "mode":"train"}
      out = y
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
      ...
      #######################################################################
      # Replace "pass" statement with your code
      cache = {"x":x, "gamma": gamma, "eps":eps, "beta": beta, "mode": "test", "running_mean": running_mean, "running_var": running_var}
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, D = dout.shape
    mode, beta, gamma, eps = cache['mode'], cache['beta'], cache['gamma'], cache['eps']
    x = cache['x']
    if(mode == 'train'):
      var, mu, x_norm = cache['var'], cache['mu'], cache['x_norm']
      #y=gamma*x_norm + beta
      dbeta = torch.sum(dout, 0)
      dgamma = torch.sum(dout * x_norm, 0)

      dx_norm = dout * gamma
      dxdmu = dx_norm * 1 / (var + eps)**(1/2)
      dvar = -torch.sum(dx_norm * (x - mu), 0) / 2 * (var + eps) ** (-3/2)
      dvdmu = 2 * (x - mu) / N * dvar #torch.ones((N, D),dtype=x.dtype,device=x.device) * dvar

      dmu = -1 * torch.sum(dxdmu + dvdmu, 0)
      dx1 = dxdmu + dvdmu
      dx2 = 1 / N * dmu # torch.ones((N, D),dtype=x.dtype,device=x.device)* dmu
      dx = dx1 + dx2
    elif(mode == 'test'):
  ...
  def backward_alt(dout, cache):
    ...
    #dYdX = dYdsigma * dsigdvar * dvardx
    mode, beta, gamma, eps = cache['mode'], cache['beta'], cache['gamma'], cache['eps']
    var, mu, x_norm, x = cache['var'], cache['mu'], cache['x_norm'], cache['x']
    std = (var + eps)**(1/2)
    N = dout.shape[0]
    dbeta = 1. * dout.sum(dim=0)
    dgamma = torch.sum(dout* x_norm, dim=0)
  
    dmudx = torch.sum(dout, dim=0)/N
    dvardx = 2/N * torch.sum((x-mu)*dout, dim=0)
    dstd_dvar = dvardx/(2*std)
    dx = gamma*((dout - dmudx)*std - dstd_dvar*(x-mu))/std**2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
