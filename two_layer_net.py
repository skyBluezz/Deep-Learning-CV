...
def nn_forward_pass(params, X):
    ...
    scores1 = torch.mm(X,W1) + b1
    hidden = torch.max(torch.zeros_like(scores1),scores1) #(N,H)
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    sum_j = torch.sum(exp_scores,dim=1)
    loss_vec = exp_scores[torch.arange(N),y] / sum_j
    loss = torch.sum(-torch.log(loss_vec)) / N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    dscores = exp_scores / torch.sum(exp_scores, axis=1, keepdim=True)
    dscores[torch.arange(N),y] -= 1
    dscores /= N

    #upstream computing..
    grads['W2'] = torch.mm(torch.transpose(hidden,0,1),dscores) #transpose hidden?
    grads['b2'] = torch.sum(dscores, dim=0)
    grads['W2'] += 2*reg*W2
    
    #backing to the hidden layer..
    dhidden = torch.mm(dscores, torch.transpose(W2,0,1))
    dhidden[hidden <= 0] = 0
    grads['W1'] = torch.mm(torch.transpose(X,0,1), dhidden)
...
def nn_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-2,0.1,1,1.3]
  hidden_sizes = [16,32,64]
  regularization_strengths = [1e-5,1e-4,4e-4,1e-3]
  learning_rate_decays = [1.0,0.95,0.90,0.85]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
    ...
    for hs in hidden_sizes:
      for reg in regularization_strengths:
        for lr in learning_rates:
          ...
          ###should i do that ^, or instead explicitly call nn_predict, and get val_acc with y_pred == y
          y_val_pred = net.predict(data_dict['X_val'])
          curr_val_acc = 100.0 * (y_val_pred == data_dict['y_val']).double().mean().item()
